...
let get_flow_id ~type_ message =
  List.find (Message.flows message :> Flows.Id.t list)
...
let add_event t message =
  ...
  | None, Some _ ->
    failwithp [%here] "invalid log entry" message Message.sexp_of_t
...
let update_inbound_envelope t message ~which =
  ...
  Option.iter inbound_envelope ~f:(fun in_ ->
    ...
    | `Data ->
      ...
      Option.iter email ~f:(fun email ->
        in_.Inbound_envelope.data <- Some email))
...
let create_outbound_envelope t message =
  ...
  Option.iter inbound_envelope ~f:(fun in_ ->
    ...
    in_.Inbound_envelope.outbound_envelopes <- out :: in_.Inbound_envelope.outbound_envelopes;
    let recipients =
      Smtp_mail_log.Message.recipients message
      |> Option.value_map ~default:[] ~f:(List.map ~f:(function
        | `String str -> str
        | `Email addr -> Email_address.to_string addr))
    in
...
let summary t =
  ...
  let emails = List.map t.inbound_envelopes ~f:(fun inb ->
    { Summary.
      subject = Inbound_envelope.get_header_value inb "Subject"
