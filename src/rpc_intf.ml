...
let or_error (type a) m
  ...
  (module struct type t = M.t Or_error.t [@@deriving bin_io] end)

let list (type a) m
  : (module Binable.S with type t = a list) =
  let module M = (val m : Binable.S with type t = a) in
  (module struct type t = M.t list [@@deriving bin_io] end)

let option (type a) m
  : (module Binable.S with type t = a option) =
  let module M = (val m : Binable.S with type t = a) in
  (module struct type t = M.t option [@@deriving bin_io] end)
...
let pair (type a) (type b) (m1, m2)
  ...
  (module struct type t = M1.t * M2.t [@@deriving bin_io] end)
...
let triple (type a) (type b) (type c) (m1, m2, m3)
  ...
  (module struct type t = M1.t * M2.t * M3.t [@@deriving bin_io] end)
