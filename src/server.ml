...
let read_mbox ?(log = Lazy.force mbox_log) reader =
  ...
  let rec read_msg () =
    match%bind Reader.read_line reader with
    | `Eof ->
      if Bigbuffer.length buffer = 0 then return `Eof else return (`Ok (message ()))
    | `Ok line ->
      if starts_new_message line
      then
        if (* Discard the "From " line since it's not part of the message body. *)
          Bigbuffer.length buffer = 0
        then read_msg ()
  ...
  let rec read_msgs out =
    match%bind read_msg () with
    | `Eof -> return ()
    | `Ok msg ->
      let%bind () = Pipe.write out msg in
      read_msgs out
  in
