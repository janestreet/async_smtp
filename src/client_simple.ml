...
let send' ?server ?(from=Email_address.local_address ()) ~to_ ?(cc=[]) ?(bcc=[]) ~subject ?id ?date ?extra_headers ?attachments content =
...
let send ?server ?from ~to_ ?cc ?bcc ~subject ?id ?date ?extra_headers ?attachments content =
  send' ?server ?from ~to_ ?cc ?bcc ~subject ?id ?date ?extra_headers ?attachments content
    >>|? Envelope_status.ok_or_error ~allow_rejected_recipients:false
    >>| Or_error.join
    >>| Or_error.ignore
